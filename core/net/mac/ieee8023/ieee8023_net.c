...
output(const uip_lladdr_t *localdest)
{
  ...
  packetbuf_set_addr(PACKETBUF_ADDR_SENDER, (linkaddr_t*)&linkaddr0_node_addr);
...
{  
   ...
   */
#if ! UIP_MULTI_IFACES
  linkaddr_copy(&linkaddr_node_addr, (linkaddr_t*)&linkaddr0_node_addr);
...
#pragma message("LINK address set by Ethernet")
  ...
  memcpy(eth_addr.addr, &linkaddr0_node_addr, UIP_LLADDR_LEN);
...
connect_event(uint8_t up)
{
  if (up) {
    ...
    if ((uip_iface = uip_ds6_register_net_iface(NETSTACK_8023, 
      (linkaddr_t* )&linkaddr0_node_addr, output)) == NULL) {
