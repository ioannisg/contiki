...
#include "net/mac/ieee8023/ieee8023_mac.h"
...
send_packet(mac_callback_t sent, void *ptr)
{
  ...
  switch(ETHERNET_STACK.tx_packet()) {
  case ETHERNET_TX_OK:
...
packet_input(void)
{
  ...
  } else if(!linkaddr0_cmp(packetbuf_addr(PACKETBUF_ADDR_RECEIVER),
      &linkaddr0_node_addr) &&
      !linkaddr0_cmp(packetbuf_addr(PACKETBUF_ADDR_RECEIVER),
      &linkaddr0_null)) {
