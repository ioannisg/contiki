...
parse(void)
{
  ...
  /* Register addresses */
  linkaddr0_t srcaddr, destaddr;

  linkaddr0_copy(&srcaddr, (linkaddr0_t*)(&hdr->et_src[0]));
  linkaddr0_copy(&destaddr, (linkaddr0_t*)(&hdr->et_dest[0]));
  ...
  if( (destaddr.u8[0] != 0x33 || destaddr.u8[0] != 0x33) &&
    !linkaddr0_cmp(&destaddr, &linkaddr0_node_addr)) {
    ...
    if (destaddr.u8[0] == 0x33 && destaddr.u8[0] == 0x33) {
      ...
      packetbuf_set_addr(PACKETBUF_ADDR_RECEIVER, (linkaddr_t*)&linkaddr0_null);
    } else {
      packetbuf_set_addr(PACKETBUF_ADDR_RECEIVER, (linkaddr_t*)&linkaddr0_node_addr);
    }
  }
...
create(void)
{
	...
	if(packetbuf_hdralloc(hdr_len)) {
      ...
      linkaddr0_copy((linkaddr0_t*)(hdr->et_src),
			(linkaddr0_t*)packetbuf_addr(PACKETBUF_ADDR_SENDER));
      linkaddr0_copy((linkaddr0_t*)(hdr->et_dest),
			(linkaddr0_t*)packetbuf_addr(PACKETBUF_ADDR_RECEIVER));
	 ...
	 */
	if(((linkaddr0_t*)packetbuf_addr(PACKETBUF_ADDR_RECEIVER))->u8[0] == 0x33 &&
		((linkaddr0_t*)packetbuf_addr(PACKETBUF_ADDR_RECEIVER))->u8[1] == 0x33) {
			PRINTF("IEEE8023_FRAMER: Reverting to standard NULL linkaddr\n");
			packetbuf_set_addr(PACKETBUF_ADDR_RECEIVER, 
				(linkaddr_t*)&linkaddr0_null);
	}
	return result;
}
/*---------------------------------------------------------------------------*/
