 ...
 * enc424j600.h
 *
 * Header file for the ENC424J600 Ethernet device API
 *
 * Created: 2014-08-18 18:18:15
...
/** Interrupt callback from the ENC424J600 peripheral */
typedef void (*enc424j600_irq_cb)(uint32_t status);

/* SPI operation status results */
enum {
  ENC424J600_STATUS_OK = 0,
  ENC424J600_STATUS_ERR,
  ENC424J600_STATUS_BUSY,
};

/* IRQ Status update flags for driver */
enum {
	ENC424J600_IRQ_STATUS_RX = 0,
	ENC424J600_IRQ_STATUS_RX_ABRT,
	ENC424J600_IRQ_STATUS_TX_COMP,
	ENC424J600_IRQ_STATUS_TX_ABRT,
	ENC424J600_IRQ_STATUS_LINK_UP,
	ENC424J600_IRQ_STATUS_LINK_DN,
};

/* ENC424J600 configuration parameters */
...
#define RXSTART   (0x1600)	// Receive buffer start (should be an even memory address)

/** Initialize the ENC424J600 module */
uint8_t enc424j600_init(void);

/** Receive an Ethernet frame available at the ENC424J600 peripheral */
uint8_t enc424j600_pkt_recv(uint16_t maxlen, uint8_t* packet, uint16_t* length);

/** Send an Ethernet frame through the ENC424J600 peripheral */
uint8_t enc424j600_pkt_send(uint16_t len, uint8_t* packet);

/** Read the MAC address at the ENC424J600 peripheral */
uint8_t enc424j600_get_macaddr(uint8_t* macAddr);

/* Set the MAC address for the ENC424J600 peripheral */
uint8_t enc424j600_set_macaddr(uint8_t* macAddr);

/** Enable power saving mode on the ENC424J600 device */
uint8_t enc424j600_powersave_enable(void);

/** Disable power saving mode on the ENC424J600 device */
uint8_t enc424j600_powersave_disable(void);

/** Return the status of the Ethernet link [connected or not] */
uint8_t enc424j600_is_linked(void);

uint8_t enc424j600_save_to_user_space(uint16_t position, uint8_t* data, uint16_t len);
uint8_t enc424j600_read_from_user_space(uint16_t position, uint8_t* data, uint16_t len);

/* Register  IRQ call-back function for asynchronous message passing to the Contiki driver */
void enc424j600_set_irq_callback(enc424j600_irq_cb);

uint8_t enc424j600_get_speed(void);
uint8_t enc424j600_get_dpx_mode(void);

/* Get receive buffer contents */
uint16_t enc424j600_get_recv_frame_len(void);
uint8_t* enc424j600_get_recv_frame_ptr(void);
...
#define RCRU         (0x20u)     // Read Control Register Un-banked
#define WCRU         (0x22u)     // Write Control Register Un-banked
#define BFS          (0x4u<<5)   // Bit Field Set
#define BFSU         (0x24u)     // Bit Field Set Un-banked
#define BFC          (0x5u<<5)   // Bit Field Clear
#define BFCU         (0x26u)     // Bit Field Clear Un-banked
