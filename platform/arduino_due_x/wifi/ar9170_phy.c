...
{	/* regpair is not pointer now JOHN*/
...
static int 
ar9170_init_power_cal(struct ar9170 *ar)
{
...
struct ar9170_phy_init {
	uint32_t reg, _5ghz_20, _5ghz_40, _2ghz_40, _2ghz_20;
};
 ...
 */
static uint32_t
ar9170_def_val(uint32_t reg, bool is_2ghz, bool is_40mhz)
{
	unsigned int i;
 ...
 */
static int 
ar9170_init_phy_from_eeprom(struct ar9170 *ar,
                                bool is_2ghz, bool is_40mhz)
{
	static const uint8_t xpd2pd[16] = {
...
static int 
ar9170_init_phy(struct ar9170 *ar, enum ieee80211_band band)
{
	int i, err;
	uint32_t val;
	...
	if (!ar->fw.hw_counters) {
		err = ar9170_write_reg(ar, AR9170_PWR_REG_PLL_ADDAC,
...
struct ar9170_rf_initvals {
	uint32_t reg, _5ghz, _2ghz;
};


static struct ar9170_rf_initvals carl9170_rf_initval[] = {
	         /* bank 0 */
...
static int 
ar9170_init_rf_banks_0_7(struct ar9170 *ar, bool band5ghz)
{
	int err, i;
	...
	for (i = 0; i < ARRAY_SIZE(carl9170_rf_initval); i++)
		ar9170_regwrite(carl9170_rf_initval[i].reg,
	...
	err = ar9170_regwrite_result();
	if (err)
		printf("AR9170_PHY: rf init failed\n");
...
struct ar9170_phy_freq_params {
	         uint8_t coeff_exp;
	         uint16_t coeff_man;
	         uint8_t coeff_exp_shgi;
	         uint16_t coeff_man_shgi;
};
...
static int 
ar9170_init_rf_bank4_pwr(struct ar9170 *ar, bool band5ghz,
                                       uint32_t freq, enum ar9170_bw bw)
{
	         int err;
	         uint32_t d0, d1, td0, td1, fd0, fd1;
	         uint8_t chansel;
	         uint8_t refsel0 = 1, refsel1 = 0;
	         uint8_t lf_synth = 0;
                            ...
                            enum ar9170_bw bw)
{
	...
	if(chanidx >= ARRAY_SIZE(ar9170_phy_freq_params)) {
		PRINTF("chanidx too large\n");
	}
	
	if(__CARL9170_NUM_BW != 3) {
		PRINTF("CARL9170_NUM_BW not 3\n");
	}
	
	if(ar9170_phy_freq_params[chanidx].freq != freq) {
		PRINTF("freq no match\n");
	}
...
static int 
ar9170_find_freq_idx(int nfreqs, uint8_t *freqs, uint8_t f)
{
	int idx = nfreqs - 2;
...
static int32_t 
ar9170_interpolate_s32(int32_t x, int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
...
static uint8_t 
ar9170_interpolate_u8(uint8_t x, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
 {
	#define SHIFT           8
	int32_t y;
...
static uint8_t 
ar9170_interpolate_val(uint8_t x, uint8_t *x_array, uint8_t *y_array)
{
	int i;
...
static int 
ar9170_set_freq_cal_data(struct ar9170 *ar, struct ieee80211_channel *channel)
{
	uint8_t *cal_freq_pier;
	uint8_t vpds[2][AR5416_PD_GAIN_ICEPTS];
	uint8_t pwrs[2][AR5416_PD_GAIN_ICEPTS];
	int chain, idx, i;
	uint32_t phy_data = 0;
	uint8_t f, tmp;
	...
	for (chain = 0; chain < AR5416_MAX_CHAINS; chain++) {
		for (i = 0; i < AR5416_PD_GAIN_ICEPTS; i++) {
			...
			for (j = 0; j < 2; j++) {
				vpds[j][i] = ar9170_interpolate_u8(f, cal_freq_pier[idx], cal_pier_data->vpd_pdg[j][i],
				                                         cal_freq_pier[idx + 1],
		...
		for (i = 0; i < 76; i++) {
			if (i < 25) {
				tmp = ar9170_interpolate_val(i, &pwrs[0][0],&vpds[0][0]);
			} else {
				tmp = ar9170_interpolate_val(i - 12, &pwrs[1][0], &vpds[1][0]);
			}
		...
		for (i = 19; i < 32; i++)
		    ar9170_regwrite(0x1c6280 + chain * 0x1000 + (i << 2), 0x0);
	    }
...
static uint8_t
ar9170_get_max_edge_power(struct ar9170 *ar, uint32_t freq, 
	struct ar9170_calctl_edges edges[])
{
	int i;
	uint8_t rc = AR5416_MAX_RATE_POWER;
	uint8_t f;
	if (freq < 3000)
		f = freq - 2300;
	else
	...
	for (i = 0; i < AR5416_NUM_BAND_EDGES; i++) {
		...
		if (i > 0 && f < edges[i].channel) {
			if (f > edges[i - 1].channel &&
				...
				rc = edges[i - 1].power_flags & ~AR9170_CALCTL_EDGE_FLAGS;
			}
			break;
		}
	}
...
static uint8_t
ar9170_get_heavy_clip(struct ar9170 *ar, uint32_t freq,
 ...
 */
static void
ar9170_calc_ctl(struct ar9170 *ar, uint32_t freq, enum ar9170_bw bw)
{
	uint8_t ctl_grp; /* CTL group */
	uint8_t ctl_idx; /* CTL index */
	int i, j;
	struct ctl_modes {
		uint8_t ctl_mode;
		uint8_t max_power;
		uint8_t *pwr_cal_data;
		int pwr_cal_len;
	} *modes;
	...
	for (i = 0; i < nr_modes; i++) {
		uint8_t c = ctl_grp | modes[i].ctl_mode;
		...
		if (ctl_idx < AR5416_NUM_CTLS) {
			...
			if (modes[i].ctl_mode == CTL_11G) {
				ar->heavy_clip = ar9170_get_heavy_clip(ar, freq, bw, EDGES(ctl_idx, 1));
			}
			
			/* adjust freq for 40MHz */
			if (modes[i].ctl_mode == CTL_2GHT40 || modes[i].ctl_mode == CTL_5GHT40) {
			...
			modes[i].max_power = ar9170_get_max_edge_power(ar, freq + f_off, EDGES(ctl_idx, 1));
			...
			while (k-- > 0) {
				if (modes[k].max_power != AR5416_MAX_RATE_POWER) {
...
static void 
ar9170_set_power_cal(struct ar9170 *ar, uint32_t freq, enum ar9170_bw bw)
{
	...
	int idx, i, n;
	uint8_t f;
	uint8_t pwr_freqs[AR5416_MAX_NUM_TGT_PWRS];
	...
	for (i = 0; i < 3; i++) {
		...
		case 2: /* 2.4 GHz OFDM */
			...
			break;
		default:
			PRINTF("AR9170_PHY: default reached\n");
		}
...
int 
ar9170_get_noisefloor(struct ar9170 *ar)
{
	static const uint32_t phy_regs[] = {
		AR9170_PHY_REG_CCA, AR9170_PHY_REG_CH2_CCA,
	    AR9170_PHY_REG_EXT_CCA, AR9170_PHY_REG_CH2_EXT_CCA };
	uint32_t phy_res[ARRAY_SIZE(phy_regs)];
	int err, i;
	
	if(ARRAY_SIZE(phy_regs) != ARRAY_SIZE(ar->noise)) {
		PRINTF("AR9170_PHY: bug on noise\n");
	}
...
nl80211_to_carl(enum nl80211_channel_type type)
{
	...
	default:
		PRINTF("AR9170_PHY: nl to carl:default\n");
	}
}

int 
ar9170_set_channel(struct ar9170 *ar, struct ieee80211_channel *channel,
                          enum nl80211_channel_type _bw)
{
	...
	/* cold reset BB/ADDA */
	err = ar9170_write_reg(ar, AR9170_PWR_REG_RESET,
	                                  AR9170_PWR_RESET_BB_COLD_RESET);
	if (err)
		return err;
	
	err = ar9170_write_reg(ar, AR9170_PWR_REG_RESET, 0x0);
	if (err)
		return err;
	...
	case CARL9170_BW_40_ABOVE:
		...
		break;
	default:
		PRINTF("AR9170_PHY: default in set channel\n");
		return -ENOSYS;
	}
	...
	err = ar9170_exec_cmd(ar, AR9170_CMD_RF_INIT, sizeof(rf), &rf,
	...
	if (err != 0) {
		...
		if (ar->chan_fail > 3) {
			...
			1702                          */
			// FIXME ar9170_restart(ar, AR9170_RR_TOO_MANY_PHY_ERRORS);
			return 0;
		}
	...
	if (ar->heavy_clip) {
		...
		if (err) {
			//if (net_ratelimit()) {
				PRINTF("failed to set heavy clip\n");
			//}
